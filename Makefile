...
.PHONY: build
build:
	CGO_ENABLED=0 GOOS=linux go build -mod=vendor -a -installsuffix cgo -o httpqueue ./cmd/main.go
...
gci: | $(GOBIN)/gci
	@gci write --Section Standard --Section Default --Section "Prefix(github.com/cubny/httpqueue)"  $(shell ls  -d $(PWD)/*/ | grep -v vendor)
